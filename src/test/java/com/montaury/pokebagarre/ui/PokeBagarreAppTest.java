...
class PokeBagarreAppTest {
    ...
    @Test
    void testMemePokemon(FxRobot robot){
        robot.clickOn(IDENTIFIANT_CHAMP_DE_SAISIE_POKEMON_1);
        robot.write("Pikachu");
        robot.clickOn(IDENTIFIANT_CHAMP_DE_SAISIE_POKEMON_2);
        robot.write("Pikachu");
    ...
    @Test
    void testPremierPokemonGagnant(FxRobot robot){
        robot.clickOn(IDENTIFIANT_CHAMP_DE_SAISIE_POKEMON_1);
        robot.write("Palkia");
        robot.clickOn(IDENTIFIANT_CHAMP_DE_SAISIE_POKEMON_2);
        robot.write("Pichu");
        ...
        await().atMost(5, TimeUnit.SECONDS).untilAsserted(() ->
            assertThat(getResultatBagarre(robot)).isEqualTo("Le vainqueur est: Palkia")
    );
    }

    @Test
    void testDeuxiemePokemonGagnant(FxRobot robot){
        robot.clickOn(IDENTIFIANT_CHAMP_DE_SAISIE_POKEMON_2);
        robot.write("Palkia");
        robot.clickOn(IDENTIFIANT_CHAMP_DE_SAISIE_POKEMON_1);
        robot.write("Pichu");
    ...
    @Test
    void testMauvaisPokemon(FxRobot robot){
        robot.clickOn(IDENTIFIANT_CHAMP_DE_SAISIE_POKEMON_1);
        robot.write("Pichu");
        robot.clickOn(IDENTIFIANT_CHAMP_DE_SAISIE_POKEMON_2);
        robot.write("GrasGrasGras");
        robot.clickOn(IDENTIFIANT_BOUTON_BAGARRE);
        await().atMost(5, TimeUnit.SECONDS).untilAsserted(() ->
                assertThat(getMessageErreur(robot)).isEqualTo("Erreur: Impossible de recuperer les details sur 'GrasGrasGras'")
        );
    }
